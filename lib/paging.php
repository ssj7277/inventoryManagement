...
class PAGING
{
    ...
    public function getPaging()
    {
        if ($this->block > 0) {
            ...
            $op = '<li class=""><a class="" href="' . $_SERVER['PHP_SELF'] . '?' . $this->tails . '&page=' . $prev_block . '"><</a></li>';
        } else {
            $op = '';
        }
        ...
        for ($i = 1; $i <= $this->scale && $this->start_page <= $this->page_max; $i++, $this->start_page++) {
            if ($this->start_page == $this->page) {
                $op .= '<li class=""><a class="" href="' . $_SERVER['PHP_SELF'] . '?' . $this->tails . '&page=' . $this->start_page . '">' . $this->start_page . '</a></li>';
            } else {
                $op .= '<li class=""><a class="" href="' . $_SERVER['PHP_SELF'] . '?' . $this->tails . '&page=' . $this->start_page . '">' . $this->start_page . '</a></li>';
            }
        }
        ...
        if ($this->page_max > ($this->block + 1) * $this->scale) {
            ...
            $op .= '<li class=""><a class="" href="' . $_SERVER['PHP_SELF'] . '?' . $this->tails . '&page=' . $next_block . '">></a></li>';
        } else {
            $op .= '';
        }
